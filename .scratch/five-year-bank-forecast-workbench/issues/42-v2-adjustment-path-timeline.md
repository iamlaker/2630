...
- [x] v2 结果区展示调整路径步骤条（顺序、优先级、变量名、起止值、硬缺口变化）
- [x] key → 指标名映射正确（找不到时回退显示 key）
- [x] 空路径显示"未发生调整"
- [x] 日志中的路径文本行保留（审计兼容）
...
- 2026-07-20 实现（ec52428）：reverseAdjustmentPath 步骤条（顺序/优先级/起止值/硬缺口变化，key→指标名映射自 variables），空路径显示「未发生调整」，仅 v2 渲染；后端日志文本保留；全套测试通过。
